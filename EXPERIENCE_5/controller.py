...
from flygym.examples.plume_tracking import OdorPlumeArena
...
class PlumeNavigationTask(HybridTurningNMF):
    ...
    def render(self, *args, **kwargs):
        ...
        mean_intensities = self.get_observation()["odor_intensity"][0]

        # right antenna
        mean_intensity = mean_intensities[3]
        mean_intensity_relative = np.clip(
            mean_intensity / self._intensity_display_vmax, 0, 1
        )
        rmin = self.cameras[0].window_size[1] - 10
        ...
        # left antenna
        mean_intensity = mean_intensities[2]
        mean_intensity_relative = np.clip(
            mean_intensity / self._intensity_display_vmax, 0, 1
        )
        rmin = self.cameras[0].window_size[1] - 20
        rmax = self.cameras[0].window_size[1] - 10
        cmin = 0
        cmax = int(self.cameras[0].window_size[0] * mean_intensity_relative)
        rendered_img[rmin:rmax, cmin:cmax] = (0, 0, 255)
